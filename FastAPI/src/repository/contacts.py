...
async def update_contact(db: AsyncSession, contact_id: UUID, contact: ContactCreate,user: User) -> Contact | None:
    stmt = select(Contact).filter_by(id=contact_id,user=user)
    result = await db.execute(stmt)
...
async def search_contacts(db: AsyncSession, query: str,user: User) -> List[Contact]:
    stmt = select(Contact).filter(
        ...
        Contact.email.ilike(f"%{query}%"),user=user
    )
    result = await db.execute(stmt)
    return result.scalars().all()


async def get_contacts_with_upcoming_birthdays(db: AsyncSession, start_date: date, end_date: date,user: User) -> List[Contact]:
    stmt = select(Contact).filter(Contact.birthday.between(start_date, end_date),user=user)
    result = await db.execute(stmt)
    return result.scalars().all()
